...
{
	return syscall(SYS_env_set_trapframe, 1, envid,
                       (uint32_t) tf, 0, 0, 0);
}
...
{
	return syscall(SYS_env_set_pgfault_upcall, 1, envid,
                       (uint32_t) upcall, 0, 0, 0);
}

int
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, int perm)
{
	return syscall(SYS_ipc_try_send, 1, envid, value,
                       (uint32_t) srcva, perm, 0);
}
